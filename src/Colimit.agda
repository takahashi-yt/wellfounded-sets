...
-- -- transportLemma : {ℓC ℓC' ℓ₁ ℓ₂ : Level} (C : WildCat ℓC ℓC')
-- --                  {f g : ob C → ob (WFWildCat ℓ₁ ℓ₂)} (p : f ≡ g) →
-- --                  {c d : ob C} (h : WFWildCat ℓ₁ ℓ₂ [ f c , f d ]) (x : typeOf' (f c)) →
-- --                  fst (transport (λ i → WFWildCat ℓ₁ ℓ₂ [ p i c , p i d ]) h)
-- --                      (transport (λ i → typeOf' (p i c)) x) ≡
-- --                    transport (λ i → typeOf' (p i d)) (fst h x)
-- -- transportLemma {ℓC = ℓC} {ℓC' = ℓC'} {ℓ₁ = ℓ₁} {ℓ₂ = ℓ₂} C {f} =
-- --   J (λ y p → {c d : ob C} (h : WFWildCat ℓ₁ ℓ₂ [ f c , f d ]) (x : typeOf' (f c)) →
-- --              fst (transport (λ i → WFWildCat ℓ₁ ℓ₂ [ p i c , p i d ]) h)
-- --                  (transport (λ i → typeOf' (p i c)) x) ≡
-- --                transport (λ i → typeOf' (p i d)) (fst h x))
-- --     (λ {c} {d} h x → cong (transp (λ i → fst (f d)) i0)
-- --                           (cong (fst h)
-- --                                 (transportRefl (transp (λ i → fst (f c)) i0 x) ∙ transportRefl x)))
...
--                    F-ob F (ordAsWF α) ≡ F-ob G (ordAsWF α)
-- dilatingProperty {F = F} {G = G} preserve α equal₁ equal₂ =
--   isoToIdWF (record { mor = {!!};
--                       inv = {!!};
--                       sec = {!!};
--                       ret = {!!}})
--   where
...
--                    (ordAsWF (colim (ordIsColimit α))) (colim c)
...
--     let mor₁ : WFWildCat ℓ-zero ℓ-zero [ F-ob (F ∘WFun (OrdToWF ∘WFun Diagram α)) u , colim (fst colimGD) ]
--         mor₁ = transport (λ i → WFWildCat ℓ-zero ℓ-zero [ fst equal₂ i u , colim (fst colimGD) ])
--                          (coconeIn (colimCocone (fst colimGD)) u)

--         mor₂ : WFWildCat ℓ-zero ℓ-zero [ F-ob (F ∘WFun (OrdToWF ∘WFun Diagram α)) u , colim (fst colimGD) ]
--         mor₂ = comp' (WFWildCat ℓ-zero ℓ-zero)
